...
if __name__ == "__main__":
    ...
    for labeled_textgrid, predicted_textgrid in zip(labeled_textgrids, predicted_textgrids):
        ...
        x_xmin = [x.xmin() for x in labeled_vots]
        x_xmax = [x.xmax() for x in labeled_vots]
        x_vot = [(x.xmax()-x.xmin()) for x in labeled_vots]

        predicted_vots = read_textgrid_tier(predicted_textgrid.strip(), args.predicted_vot_tier)
        y_xmin = [y.xmin() for y in predicted_vots]
        y_xmax = [y.xmax() for y in predicted_vots]
        y_vot = [(y.xmax()-y.xmin()) for y in predicted_vots]
