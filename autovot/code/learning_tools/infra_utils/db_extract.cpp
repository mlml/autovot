...
int main(int argc,char* argv[]) 
{
  ...
  cmd_line cmdline;
  cmdline.info("Create new db and labels files from a given db and labels files.\n"
	       "The vector are taken in order from place {from} to place {to}.\n");
  cmdline.add_master_option("in_db_file(no extension)", &input_filename);
  cmdline.add_master_option("out_db_file(no extension)", &output_filename);
  cmdline.add("-from", "first vector to be extracted", &from, 1);
  cmdline.add("-to", "last vector to be extracted", &to, 9999999);
