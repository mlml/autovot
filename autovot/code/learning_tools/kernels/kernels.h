...
// Revised by: Joseph Keshet (chi2 and cs_gaussian)
//
//=============================================================================
...
namespace kernels {
  ...
  //*****************************************************************************
  /** Implements a cs_gaussian kernel
   */
  class cs_gaussian : public BaseKernel {
  public:
    //-----------------------------------------------------------------------------
    /** Constructor K(x,y) = v(0) * exp( - ||x-y||^2 / 2 v(1)) * [ max{ 1 - ||x-y||/v(2) , 0} ]^v(3)
     */
    cs_gaussian(infra::vector_view v) : BaseKernel(v) {
      bool wrong_parameters = false;
      if (v.size() != 4) {
        wrong_parameters = true;
      } else if (v(0) <= 0 || v(1) <= 0 || v(2) <= 0) {
      ...
      if (wrong_parameters) {
        std::cerr << "The parameter vector:\n" << v << "is wrong for compactly supported gaussian kernel.\n" 
        << "Aborting." << std::endl;
        exit(EXIT_FAILURE);
      }
      
      _s = log(param_(0)); _sigma2  = param_(1); _cutoff = param_(2); _nu = param_(3);
    }
  ...
  public:
    ...
    enum MyKernelsType { LINEAR = 0 , POLYNOMIAL = 1, GAUSSIAN = 2, CHI2 = 3, CS_GAUSSIAN = 4 };
    ...
    KernelsBuilder(std::string file_name) : K(NULL), smart_pointer_counter(NULL) {
      if (file_name == "default") {
        ...
        if (s == "linear") {
          ...
          kernel_code = CHI2;
        } else if (s == "cs_gaussian") {
          kernel_code = CS_GAUSSIAN;
        }
  ...
  protected:
    ...
    void AllocateBaseKernel(short kernel_code, infra::vector_view v) {
      ...
      if (kernel_code == LINEAR) {
        ...
        K = new chi2(v);        
      } else if (kernel_code == CS_GAUSSIAN) {
        K = new cs_gaussian(v);
      } else {
