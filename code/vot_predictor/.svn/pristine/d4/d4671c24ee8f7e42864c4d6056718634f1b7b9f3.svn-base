...
#ifdef _USE_MPI_
  //-----------------------------------------------------------------------------
  /** Reduces matrices on all processes to a single value
   @param one of a number of MPI_Op functions to be performed on the matrix 
   @return MPI error code
   */
  inline int mpi_reduce(MPI_Op op);
  
  //-----------------------------------------------------------------------------
  /** Broadcasts the matrix from root to all tasks
   @param the root task that will broadcast the matrix 
   @return MPI error code
   */
  inline int mpi_bcast(int root);

  //-----------------------------------------------------------------------------
  /** Performs a blocking send
   @param The task number the matrix will be send to
   @param a message tag
   */
  inline int mpi_send(int dest, int tag);

  //-----------------------------------------------------------------------------
  /** Blocking receive
   @param The SOURCE task number 
   @param a message tag
   */
  inline int mpi_recv(int source = MPI_ANY_SOURCE, int tag = MPI_ANY_TAG);
#endif  
