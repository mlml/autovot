...
void Instances::read(std::string &filename, bool labels_given)
{
  ...
  if (!ifs.good()) {
    std::cerr << "Error: Unable to read instances from " << filename << std::endl;
    exit(-1);
  } 
  ...
  while (!ifs.eof()) {
    ...
    if (strcmp(line,"")) {
      ...
      if (labels_given && ( _word_start > _vot_burst || _vot_burst > _word_end) ) {
        std::cerr << "Error: problem in line number " << line_num <<  " of the input filelist " << filename << std::endl;
        std::cerr << "       should be   word_start <= vot_burst < word_end" << std::endl;
        std::cerr << "       but read word_start=" << _word_start << " vot_burst=" << _vot_burst 
          << " vot_voice=" << _vot_voice << " word_end=" << _word_end << std::endl;
        exit(-1);        
      }
  ...
  while (!ifs.eof()) {
    ...
    if (strcmp(line,"")) {
      ...
      double _vot_voice;
			...
			if (token== NULL) {
				std::cerr << "Error: Unable to read instances from " << filename << " at line " << line_num << " (1)" << std::endl;
				exit(-1);
			}
...
void SpeechUtterance::read(std::string &filename)
{
  ...
  if (ifs.good()) {
    ...
    std::cerr << "Error: Unable to read instance from " << filename << std::endl;
    exit(-1);
  } 
}
...
Dataset::Dataset(std::string& instances_filelist, std::string& labels_filename)
{
  ...
  if (labels_filename == "null") {
    std::cerr << "Info: no labels were given." << std::endl;
    read_labels = false;
  }
  else {
		...
		if (!ifs_labels.good()) {
			std::cerr << "Error: unable to open " << labels_filename << " for reading" << std::endl;
			exit(-1);
		}
		...
		if ( labels.height() != instances_file_list.size() || labels.width() != 2 ) {
			std::cerr << "Error: the width of the matrix in labels file should be " 
				  << instances_file_list.size() << "x 2 ." << std::endl;
			exit(-1);
		}
    read_labels = true;
  }
...
unsigned int Dataset::read(SpeechUtterance &x, VotLocation &y)
{
	...
	<< instances_file_list[current_file] << std::endl;
