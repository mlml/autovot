...
int main(int argc, char **argv)
{
	...
	// cmdline.add("-test_only", &test_only, false);
	...
	for (unsigned int i=0; i < instances.size(); i++) {
		...
		if (instances.word_end[i] == -1) {
			instances.word_end[i] = samples.size()-1;
		}
		...
		if (labels_given) {
			...
			if (vot_offset >= int(samples.size())) {
				cerr << "Error: burst onset or voice onset: (" << vot_offset << ") is larger than " ;
				cerr << "the number of samples in " << instances.file_list[i] << endl;
				return EXIT_FAILURE;
			}
		}
		...
		infra::vector word_samples = samples.subvector(instances.word_start[i],
		...
		if (labels_given) {
			// onset frame
			int ind3 = 0;
			while (t[ind3] < double(instances.vot_burst[i])/sampling_rate)
				ind3++;
			
			// word-end frame
			int ind4 = t.size()-1;
			while (t[ind4] > double(instances.vot_voice[i])/sampling_rate)
				ind4--;
